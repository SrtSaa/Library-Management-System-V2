...
const viewReturnedBooks = {
    template: `
    <div>
        <adminNavbar title="Returned eBooks" v-if="role == 'admin'"/>
        <userNavbar title="Returned eBooks" v-else/>
                    ...
                    <table class="table table-striped table-light table-hover align-middle">
                        <thead class="table-light">
                            <tr class="text-center">
                                ...
                                <th scope="col" v-if="role == 'admin'">UserName</th>
                        ...
                        <tbody class="table-group-divider">
                            <tr class="text-center" v-for="(record, index) in records" v-if="isSearchStringPresent(record) && (role == 'admin' || (role == 'user' && record.username == username))">
                                <th scope="row">{{ index+1 }}</th>
                                <td>{{ record.ebook_id }}</td>
                                <td>{{ record.title }}</td>
                                <td v-if="role == 'admin'">{{ record.username }}</td>
                                ...
                                <td>
                                    ...
                                    <button class="btn btn-success my-1" @click="openUserDetails(record)" v-if="role == 'admin'">User Details</button> 
                                </td>
                            </tr>
                        </tbody>
                    </table>
    ...
    async mounted() {
        var res = await fetch('/api/allReturnedBooks', {
            headers: {
                'Authentication-Token': localStorage.getItem('auth-token'),
            },
        });
